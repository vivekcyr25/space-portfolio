  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        isScrolled ? "py-4 backdrop-blur-3xl bg-black/40 border-b border-white/5" : "py-8 bg-transparent"
      }`}
    >
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-[#030014]/98 backdrop-blur-3xl p-12 flex flex-col gap-10 md:hidden"
          >
            ...
            {navLinks.map((link) => (
              <a
                ...
                className="text-5xl font-black heading-syne transition-all uppercase tracking-tighter liquid-text liquid-text-strong liquid-text-hover"
              >
                {link.name}
              </a>
            ))}
            <a href="#contact" onClick={() => setIsMobileMenuOpen(false)} className="btn-primary mt-6 text-center liquid-text-hover">
              <span className="liquid-text liquid-text-subtle liquid-text-hover">Initiate Link</span>
            </a>
          </motion.div>
        )}
